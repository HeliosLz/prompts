...
build_instruction_block() {
  ...
  if pipeline_exists "run-when-committing"; then
    routes+=("- Commit requests (\`commit\`, \`提交\`, \`git commit\`): use \`.pensieve/pipelines/run-when-committing.md\`. Check staged diff, decide whether reusable insight should be captured, then make atomic commits.")
  fi

  if pipeline_exists "run-when-refactoring"; then
    routes+=("- Refactor requests (\`重构\`, \`refactor\`, \`大改\`, \`拆代码\`): use \`.pensieve/pipelines/run-when-refactoring.md\`. Confirm the real problem, fix upstream data authority first, split large work into 2-3 user-visible steps, delete old paths when new paths work, and avoid compatibility/fallback branches.")
  fi

  if pipeline_exists "run-when-reviewing-code"; then
    routes+=("- Review requests (\`review\`, \`代码审查\`, \`检查代码\`): use \`.pensieve/pipelines/run-when-reviewing-code.md\`. Start from git history and changed hot spots, verify candidate issues, and report only high-signal findings with evidence and file locations.")
  fi
